...
use slozhn_frame::http::{headers_to_metadata, metadata_to_headers, trailers_to_status, RecvBody};
...
where
    ...
    S::Error: std::fmt::Display + Send,
{
    let Incoming { method, metadata, send, recv } = inc;

    let mut builder = http::Request::builder().method(http::Method::POST).uri(&method);
    ...
    *headers = metadata_to_headers(&metadata);
    ...
    if send.send_headers(headers_to_metadata(&parts.headers)).is_err() {
        return;
    }
    ...
    loop {
        match body.frame().await {
            ...
            Some(Err(_)) => {
                let _ = send.finish(Status::with_code(13, "response body error")).await;
                return;
            }
            None => {
                // gRPC requires a terminal status; body ended without trailers
                let _ = send.finish(Status::with_code(13, "missing grpc trailers")).await;
                return;
            }
        }
    }
